...
static char author[100], title[100];
...
void load(FILE *fp)
{
	...
	while (1) {
		...
		do {
			...
			if (buf[0] == '*')
			{
				strncpy(author, &buf[1], strlen(buf)-2);
				puts("author: ");
				puts(author);
				continue;
			}
			if (buf[0] == '%')
			{
				strncpy(title, &buf[1], strlen(buf)-2);
				puts("title: ");
				puts(title);
				continue;
			}
...
void draw_header()
{
	char status[255];
	/*
	 * Size 4 stands for 3 digits and a \0 char.
	 **/
	char act_index[4];
	char max_index[4];
	sprintf(act_index, "%03d", idx);
	sprintf(max_index, "%03d", slidecount-1);
	strcpy(status, act_index);
	strcat(status, "/");
	strcat(status, max_index);
	strcat(status, " | ");
	strncat(status, author, strlen(author));
	strcat(status, " : ");
	strncat(status, title, strlen(title));
	/* Some Xft reconfig... */
	Colormap cmap = DefaultColormap(d->dpy, d->screen);
	Visual *vis = DefaultVisual(d->dpy, d->screen);
	XftDraw *drw = XftDrawCreate(d->dpy, d->drawable, vis, cmap);
	Fnt usedfont = *d->fonts;
	usedfont.xfont = XftFontOpen(d->dpy,
			d->screen,
			XFT_FAMILY,
			XftTypeString,
			fontfallbacks[0],
